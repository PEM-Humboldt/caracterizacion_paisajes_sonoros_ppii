...
dic_sound_categories = {'ALTVOZ': 'ANT',
                        'ANIDOM': 'ANT',
                        'AVEVOC': 'BIO',
                        'FLUAGU': 'GEO',
                        'HERPET': 'BIO',
...
fpath = '../manual_annotations/annot_random_sampling/'
...
df = pd.DataFrame()
for dir_name in os.walk(fpath):
    sensor_name = dir_name[0][-4:]
    flist_site = glob.glob(os.path.join(fpath,sensor_name)+'/*.txt')
    for fname_site in flist_site:
        df_aux = pd.read_csv(fname_site, sep='\t', encoding_errors='replace')
        ...
        # add fname column
        df_aux['fname'] = os.path.basename(fname_site).replace('.Table.1.selections.txt', '.WAV')
        df_aux['fname_annot'] = os.path.basename(fname_site)
        df_aux['sensor_name'] = sensor_name
        # append
        df = df.append(df_aux)
...
plt_data = prop_soundscape_id.drop(['MURCIE?', 'INDETER', 'PULSOS', 'MOL_SP', 'PASOS', 'INDETE'])
...
plt_data['color'] = np.concatenate([np.repeat('#CD6155', 3), np.repeat('#229954', 5), np.repeat('#2E86C1', 2)])
...
plt_data.drop(index=['MURCIE?', 'INDETER', 'PULSOS', 'MOL_SP', 'PASOS', 'INDETE'], inplace=True)
