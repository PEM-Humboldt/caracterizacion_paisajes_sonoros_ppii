...
#%% Multiple plots by season
tfbins_t1 = pd.read_csv('./dataframes/tfbins_aguas_altas_t1.csv')
tfbins_t2 = pd.read_csv('./dataframes/tfbins_aguas_bajas_t2.csv')

tfbins_t1 = tfbins_t1.mean(axis=0)
tfbins_t2 = tfbins_t2.mean(axis=0)
tfbins_diff = tfbins_t1 - tfbins_t2

# plot
plt.close('all')
fig, ax = plt.subplots(nrows=1, ncols=2, sharex=True, sharey=True, figsize=(8, 3.5))

ax[0].imshow(tfbins_t1.values.reshape([24, 128]).T, aspect='auto', origin='lower', 
             extent=[0, 24, 0, 24], interpolation='bilinear')
ax[0].set_xticks([0, 6, 12, 18, 24])
ax[0].set_title('T1')
ax[0].set_xlabel('Time (h)')

pos = ax[1].imshow(tfbins_t2.values.reshape([24, 128]).T, aspect='auto', origin='lower', 
             extent=[0, 24, 0, 24], interpolation='bilinear')
...
df_ind = pd.read_csv('./dataframes/indicator_species_indval_t1_t2.csv')
idx_names = ['X' + str(idx) for idx in np.arange(1, 3072+1)]
